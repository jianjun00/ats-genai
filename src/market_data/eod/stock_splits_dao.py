...
class StockSplitsDAO:
    ...
    async def insert_split(self, ex_date, split_ratio, split_from, split_to, symbol=None, instrument_id=None, source=None, vendor_id=None, at_date=None):
        """
        Insert a stock split. Prefer instrument_id; if only symbol is given, resolve instrument_id from instrument_xref.
        """
        ...
        try:
            async with pool.acquire() as conn:
                await conn.execute(f"""
                    INSERT INTO {self.table_name} (symbol, instrument_id, ex_date, split_ratio, split_from, split_to, source)
                    VALUES ($1, $2, $3, $4, $5, $6, $7)
                """, symbol, instrument_id, ex_date, split_ratio, split_from, split_to, source)
        finally:
            await pool.close()

    async def list_splits(self, symbol: Optional[str] = None, instrument_id: Optional[int] = None):
        pool = await asyncpg.create_pool(self.db_url)
        try:
            async with pool.acquire() as conn:
