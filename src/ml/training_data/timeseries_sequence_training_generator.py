...
class SequenceTrainingExample:
    ...
    # Sequence features (arrays) - past N intervals
    sequence_5m: List[Dict[str, float]]  # Past 52 x 5-minute OHLCV
    sequence_15m: List[Dict[str, float]]  # Past 52 x 15-minute OHLCV
    sequence_1h: List[Dict[str, float]]   # Past 24 x 1-hour features
    sequence_1d: List[Dict[str, float]]   # Past 20 x daily features
    
    # Multi-timeframe aggregated features
    timeframe_features: Dict[str, Dict[str, float]]  # {timeframe: {feature: value}}
    
    # Target labels (future M intervals)
    future_1h: List[Dict[str, float]]    # Next 6 x 1-hour targets
    future_1d: List[Dict[str, float]]    # Next 5 x daily targets
    
    # Metadata
    sequence_length: Dict[str, int]
    prediction_horizon: Dict[str, int]
...
class SequenceWindowBuilder:
    ...
    def get_timeframe_data(self, instrument_id: int, center_date: date, 
                          timeframe: str, window_size: int, is_future: bool = False) -> List[Dict[str, float]]:
        """
        Get data for a specific timeframe around center_date.
        
        Args:
            instrument_id: Target instrument
            center_date: Center date for the window
        ...
        try:
            if is_future:
                # Get future data using lead_prices
                data_df = self.universe_manager.get_lead_prices(instrument_id, center_date, window_size)
            else:
                # Get historical data using lag_prices  
                data_df = self.universe_manager.get_lag_prices(instrument_id, center_date, window_size)
    ...
    def build_sequence_features(self, instrument_id: int, prediction_timestamp: datetime) -> Dict[str, List[Dict[str, float]]]:
        """Build sequence features for all configured timeframes."""
        prediction_date = prediction_timestamp.date()
        sequence_features = {}
        
        # Build past sequences
        for timeframe, sequence_length in self.config.sequence_lengths.items():
            sequence_data = self.get_timeframe_data(
                instrument_id, prediction_date, timeframe, sequence_length, is_future=False
            )
            sequence_features[f'sequence_{timeframe}'] = sequence_data
        
        return sequence_features
    
    def build_prediction_targets(self, instrument_id: int, prediction_timestamp: datetime) -> Dict[str, List[Dict[str, float]]]:
        ...
        for timeframe, horizon_length in self.config.prediction_horizons.items():
            target_data = self.get_timeframe_data(
                instrument_id, prediction_date, timeframe, horizon_length, is_future=True
            )
...
class TimeSeriesSequenceTrainingGenerator:
    ...
    def __init__(self, 
        ...
        self.universe_manager = universe_manager or UniverseStateManager(env=self.env)
    ...
    async def generate_training_example(self, 
        ...
        try:
            ...
            sequence_features = self.sequence_builder.build_sequence_features(instrument_id, prediction_timestamp)
            
            # Build prediction targets
            targets = self.sequence_builder.build_prediction_targets(instrument_id, prediction_timestamp)
    ...
    def export_to_formats(self, 
                         examples: List[SequenceTrainingExample],
                         output_dir: str,
                         formats: List[str] = ['pickle', 'parquet', 'json']) -> Dict[str, str]:
        """
        Export training examples to multiple formats.
        
        Args:
            examples: List of training examples to export
            output_dir: Output directory path
            formats: List of formats to export ('pickle', 'parquet', 'json')
        ...
        # Export pickle format (preserves full object structure)
        if 'pickle' in formats:
            pickle_file = output_path / f"timeseries_training_data_{timestamp}.pkl"
            with open(pickle_file, 'wb') as f:
                pickle.dump(examples, f)
            exported_files['pickle'] = str(pickle_file)
        
        # Export parquet format (efficient for large datasets)
        if 'parquet' in formats:
            try:
                # Flatten examples to DataFrame
                flattened_data = []
                for example in examples:
                    ...
                    # Add timeframe features (flattened)
                    for tf, features in example.timeframe_features.items():
                        for feat_name, feat_val in features.items():
                            row[f'tf_{tf}_{feat_name}'] = feat_val
                    
                    # Add sequence lengths and prediction horizons as metadata
                    for tf, length in example.sequence_length.items():
                        row[f'seq_len_{tf}'] = length
                    
                    flattened_data.append(row)
                
                df = pd.DataFrame(flattened_data)
                parquet_file = output_path / f"timeseries_training_data_{timestamp}.parquet"
                df.to_parquet(parquet_file, index=False)
                exported_files['parquet'] = str(parquet_file)
                
            except Exception as e:
                self.logger.warning(f"Failed to export parquet format: {e}")
            ...
            try:
                ...
                for example in examples:
                    json_example = {
                        ...
                        'sequence_data_shapes': {
                            'sequence_5m': len(example.sequence_5m),
                            'sequence_15m': len(example.sequence_15m),
                            'sequence_1h': len(example.sequence_1h),
                            'sequence_1d': len(example.sequence_1d),
                        }
                    }
                    json_data.append(json_example)
                
                json_file = output_path / f"timeseries_training_data_{timestamp}.json"
                with open(json_file, 'w') as f:
                    json.dump(json_data, f, indent=2)
                exported_files['json'] = str(json_file)
                
            except Exception as e:
                self.logger.warning(f"Failed to export JSON format: {e}")
        
        self.logger.info(f"Exported training data to {len(exported_files)} formats in {output_dir}")
