...
class StockSplitsTiingoDAO:
    def __init__(self, env: Environment):
        self.env = env
        self.table_name = self.env.get_table_name('stock_splits_tiingo')
        ...
        async with pool.acquire() as conn:
            await conn.execute(f"""
                INSERT INTO {self.table_name} (
                    symbol, execution_date, split_from, split_to, cash_amount, declaration_date, payment_date, record_date, description, refid
