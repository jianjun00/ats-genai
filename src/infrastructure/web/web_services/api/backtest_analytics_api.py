    ...
    try:
        ...
        with get_raw_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                ...
                cursor.execute(f"""
                    SELECT backtest_run_id, strategy_name, strategy_type, universe_size,
                           start_date, end_date, status, total_return, sharpe_ratio,
                ...
                if not backtest_records:
                    return []  # Return empty list if no backtests found

                return [
                    BacktestSummary(**dict(record))
...
async def get_forecasts(
    ...
    try:
        ...
        with get_raw_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                # Build dynamic query
                where_clauses = ["backtest_run_id = %s"]
                ...
                cursor.execute(f"""
                    SELECT date, symbol, support_levels, resistance_levels,
                           support_confidence, resistance_confidence,
                           actual_low, actual_high
                    FROM dev_forecast_data
                    WHERE {where_sql}
                    ORDER BY date DESC
                """, params)
                ...
                if not forecast_records:
                    return []  # Return empty list if no forecasts found

                return [
                    ForecastDataPoint(**dict(record))
