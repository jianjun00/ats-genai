-- Migration 042: Create vendor-specific minute bar tables for real-time collection
...
CREATE TABLE IF NOT EXISTS dev_one_minute_live_polygon (
    ...
    data_latency_ms INTEGER, -- Milliseconds between bar close and receipt
    collection_method VARCHAR(20) DEFAULT 'websocket', -- websocket, polling, backfill
    is_realtime BOOLEAN DEFAULT TRUE,
    
    -- Data quality and validation
    quality_score NUMERIC(3,2) DEFAULT 0.8,
    validation_status VARCHAR(20) DEFAULT 'pending', -- pending, valid, invalid, review
...
CREATE TABLE IF NOT EXISTS dev_realtime_gaps (
    ...
    -- Gap analysis
    gap_type VARCHAR(20) NOT NULL, -- connection_loss, api_error, market_closure, weekend
    detection_method VARCHAR(20) DEFAULT 'realtime', -- realtime, batch_validation, manual
...
SELECT 
    ...
    rcs.total_bars_today,
    rcs.successful_collections,
    rcs.failed_collections,
    rcs.avg_latency_ms,
...
GROUP BY 
    ...
    rcs.consecutive_missing_bars, rcs.total_bars_today, rcs.successful_collections,
    rcs.failed_collections, rcs.avg_latency_ms, rcs.collection_health_score,
