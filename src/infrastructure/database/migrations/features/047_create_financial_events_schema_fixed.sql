-- Financial Events Database Schema (Fixed Version)
-- Professional-grade schema following Bloomberg/Refinitiv patterns
-- Created: 2025-08-27
-- Fixed to work with existing dev_instruments table structure

-- First, add primary key to dev_instruments if it doesn't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Add primary key to dev_instruments
        ALTER TABLE dev_instruments ADD PRIMARY KEY (id);
        RAISE NOTICE 'Added primary key to dev_instruments table';
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_earnings_events_instrument_period 
...
CREATE INDEX IF NOT EXISTS idx_analyst_ratings_instrument 
...
CREATE INDEX IF NOT EXISTS idx_event_impacts_instrument 
...
WITH latest_ratings AS (
    SELECT 
        ...
        ar.analyst_firm,
        ar.new_rating,
        ar.new_price_target_cents,
        ar.created_at,
        ROW_NUMBER() OVER (
            PARTITION BY ar.instrument_id, ar.analyst_firm 
            ORDER BY ar.created_at DESC
        ) as rn
    FROM dev_analyst_ratings ar
    JOIN dev_financial_events fe ON ar.financial_event_id = fe.id
    WHERE ar.created_at >= CURRENT_DATE - INTERVAL '365 days'
)
SELECT 
    instrument_id,
    symbol,
...
SELECT 'Financial Events Database Schema Created Successfully!' as status,
       'Tables: ' || COUNT(*) || ' created' as table_count
FROM information_schema.tables 
WHERE table_name LIKE 'dev_%events%' OR table_name LIKE 'dev_%actions%' OR table_name LIKE 'dev_%ratings%';