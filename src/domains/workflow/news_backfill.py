...
Basic backfill from August 25, 2025:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25

Backfill specific date range:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --end-date 2025-09-01

Backfill for specific symbols only:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --symbols AAPL,TSLA,MSFT

Limited backfill for testing:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --limit-per-request 50 --max-requests 5

Debug mode with detailed logging:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --debug

Test mode without API calls or database writes:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --dry-run

Production environment:
    PYTHONPATH=src python -m domains.workflow.polygon.backfill.news_backfill --start-date 2025-08-25 --environment prod
