...
from EODHD API, storing data in monthly Parquet files.

Features:
- Checkpoint-based resumable processing for massive scale
- Rate limiting compliant with EODHD API constraints
- File-based parquet storage
- Progress tracking and error recovery
- Quality validation and gap detection
- Supports both full and incremental population modes

Usage:
    PYTHONPATH=src python -m domains.workflow.eodhd.backfill.minute_bars_30year --mode full --limit 10
    PYTHONPATH=src python -m domains.workflow.eodhd.backfill.minute_bars_30year --mode incremental --symbols AAPL,MSFT
    PYTHONPATH=src python -m domains.workflow.eodhd.backfill.minute_bars_30year --resume --checkpoint-file last_run.json
"""
