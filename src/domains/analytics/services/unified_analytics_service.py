            ...
            <script>
                ...
                async function loadTrainingDatasets() {
                    ...
                    try {
                        ...
                        if (data.datasets) {
                            let html = `
                                <h3>🤖 Training Datasets with ML Analytics</h3>
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    <h4>Available Datasets (${data.datasets.length} total)</h4>
                            `;
                            if (data.datasets.length > 0) {
                                data.datasets.forEach(dataset => {
                                    html += `
                                        <div style="padding: 10px; border-bottom: 1px solid #eee; margin: 10px 0;">
                                            <div style="font-weight: bold; color: #1976d2;">Dataset ID: ${dataset.id}</div>
                                            <div>Symbols: ${dataset.symbols}</div>
                                            <div>Date Range: ${dataset.date_range}</div>
                                            <div style="color: #666; font-size: 0.9em;">Created: ${dataset.created_at}</div>
                                        </div>
                                    `;
                                });
                            } else {
                                html += `
                                    <div style="padding: 20px; text-align: center; color: #666;">
                                        <p>No training datasets available yet.</p>
                                        <p><em>Training datasets will be generated by the ML pipeline.</em></p>
                                    </div>
                                `;
                            }
                            html += `
                ...
                async function loadBarCollectionMetrics() {
                    ...
                    try {
                            ...
                            const html = `
                                ...
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    ...
                                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin: 20px 0;">
                                        <div style="text-align: center; padding: 15px; background: #e3f2fd; border-radius: 4px;">
                                            <div style="font-size: 1.8em; color: #1976d2;">${data.summary.total_bars_collected.toLocaleString()}</div>
                                            <div style="font-weight: bold;">Total Bars</div>
                                        </div>
                                        <div style="text-align: center; padding: 15px; background: #e8f5e8; border-radius: 4px;">
                                            <div style="font-size: 1.8em; color: #388e3c;">${data.summary.total_unique_symbols}</div>
                                            <div style="font-weight: bold;">Unique Symbols</div>
                                        </div>
                                        <div style="text-align: center; padding: 15px; background: #fff3e0; border-radius: 4px;">
                                            <div style="font-size: 1.8em; color: #f57c00;">${data.metrics.data_quality}%</div>
                                            <div style="font-weight: bold;">Data Quality</div>
                                        </div>
                                    </div>
                                    <p><strong>Last Update:</strong> ${data.summary.last_update}</p>
                                    <p><strong>Collection Rate:</strong> ${data.metrics.bars_per_hour.toLocaleString()} bars/hour</p>
                                </div>
                            `;
                            document.getElementById('analysis-content').innerHTML = html;
                        }
                        ...
                        document.getElementById('analysis-content').innerHTML =
                            '<h3>📈 Bar Collection Metrics</h3><p style="color: red;">Error loading metrics: ' + error.message + '</p>';
                    }
                }
                    ...
                    try {
                        ...
                        if (data.success) {
                            let html = `
                                ...
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    <h4>Recent News Events (${data.total_events} total)</h4>
                                    <div style="margin: 15px 0;">
                            `;
                            data.events.forEach(event => {
                                html += `
                                    <div style="padding: 10px; border-bottom: 1px solid #eee; margin: 10px 0;">
                                        <div style="font-weight: bold; color: #1976d2;">${event.symbol}</div>
                                        <div>${event.title}</div>
                                        <div style="color: #666; font-size: 0.9em;">${event.date}</div>
                                    </div>
                                `;
                            });
                            html += `
                ...
                async function loadEarningsEvents() {
                    document.getElementById('analysis-content').innerHTML =
                        '<h3>📊 Earnings Events</h3><p>Loading earnings events...</p>';
                    try {
                        ...
                        if (data.success) {
                            let html = `
                                ...
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    <h4>Upcoming & Recent Earnings (${data.total_events} total)</h4>
                                    <div style="margin: 15px 0;">
                            `;
                            data.events.forEach(event => {
                                html += `
                                    <div style="padding: 10px; border-bottom: 1px solid #eee; margin: 10px 0;">
                                        ...
                                        <div>EPS Actual: ${event.eps_actual ? '$' + event.eps_actual : 'TBD'}</div>
                                        <div style="color: #666; font-size: 0.9em;">${event.date}</div>
                                    </div>
                                `;
                            });
                ...
                async function loadMultiPanelVisualization() {
                    document.getElementById('analysis-content').innerHTML =
                        '<h3>🎨 Multi-Panel Trading Charts</h3><p>Loading visualization...</p>';
                    try {
                        const response = await fetch('/api/multi-panel-viz');
                        const data = await response.json();
                        if (data.success) {
                            const html = `
                                ...
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    ...
                                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin: 20px 0;">
                                        <div style="text-align: center; padding: 15px; background: #f5f5f5; border-radius: 4px;">
                                            ...
                                            <div style="font-weight: bold;">OHLC Charts</div>
                                        </div>
                                        <div style="text-align: center; padding: 15px; background: #f5f5f5; border-radius: 4px;">
                                            <div style="font-size: 1.5em;">📊</div>
                                            <div style="font-weight: bold;">Volume Charts</div>
                                        </div>
                                        <div style="text-align: center; padding: 15px; background: #f5f5f5; border-radius: 4px;">
                                            <div style="font-size: 1.5em;">📉</div>
                                            <div style="font-weight: bold;">Technical Indicators</div>
                                        </div>
                                    </div>
                                    <p>${data.data.message}</p>
                                    <p><em>Charts: ${data.charts.join(', ')}</em></p>
                                </div>
                            `;
                            document.getElementById('analysis-content').innerHTML = html;
                        }
                        ...
                        document.getElementById('analysis-content').innerHTML =
                            '<h3>🎨 Multi-Panel Trading Charts</h3><p style="color: red;">Error loading charts: ' + error.message + '</p>';
                    }
                }
                async function loadRayAnalytics() {
                    ...
                    try {
                        const response = await fetch('/api/ray-analytics/1');
                        const data = await response.json();
                        if (data.analysis_type) {
                            const html = `
                                <h3>⚡ Distributed Analytics with Ray</h3>
                                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                                    <h4>Ray Computing Status</h4>
                                    <div style="margin: 15px 0;">
                                        <div style="padding: 15px; background: #fff3e0; border-radius: 4px; margin: 10px 0;">
                                            <div style="font-weight: bold; color: #f57c00;">⚠️ Ray Not Available</div>
                                            <div>Falling back to local computation methods</div>
                                        </div>
                                        <div style="padding: 15px; background: #e8f5e8; border-radius: 4px; margin: 10px 0;">
                                            <div style="font-weight: bold; color: #388e3c;">✅ Local Analytics Active</div>
                                            <div>Using traditional pandas/numpy for data processing</div>
                                        </div>
                                    </div>
                                    <p><strong>Analysis Method:</strong> ${data.analysis_type || 'local'}</p>
                                    <p><strong>Computation Mode:</strong> ${data.method || 'traditional'}</p>
                                    <div style="margin: 15px 0; padding: 10px; background: #f5f5f5; border-radius: 4px;">
                                        <p><em>To enable Ray distributed computing, install Ray and configure the cluster.</em></p>
                                    </div>
                                </div>
                            `;
                            document.getElementById('analysis-content').innerHTML = html;
                        ...
                        document.getElementById('analysis-content').innerHTML =
                            '<h3>⚡ Distributed Analytics</h3><p style="color: red;">Error loading Ray analytics: ' + error.message + '</p>';
                    }
                }
            </script>
        </body>
...
class UnifiedAnalyticsRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif self.path.startswith('/api/bar-collection-metrics'):
    ...
    def _serve_tables_list(self):
        """Serve list of available database tables."""
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
    ...
    def _serve_news_events(self):
        ...
        response = {
            "success": True,
            "events": [
                {"symbol": "AAPL", "title": "Apple announces new product", "date": "2025-09-09"},
                {"symbol": "TSLA", "title": "Tesla earnings report", "date": "2025-09-08"}
            ],
            "total_events": 2
        }
        self.wfile.write(json.dumps(response).encode('utf-8'))
    ...
    def _serve_earnings_events(self):
        ...
        response = {
            "success": True,
            "events": [
                {"symbol": "AAPL", "eps_estimate": 2.50, "eps_actual": 2.65, "date": "2025-09-09"},
                {"symbol": "MSFT", "eps_estimate": 3.10, "eps_actual": None, "date": "2025-09-10"}
            ],
            "total_events": 2
        }
        self.wfile.write(json.dumps(response).encode('utf-8'))
    ...
    def _serve_bar_collection_metrics(self):
        ...
        response = {
            "success": True,
            "summary": {
                "total_bars_collected": 1250000,
                "total_unique_symbols": 3500,
                "last_update": "2025-09-09T02:00:00Z"
            },
            "metrics": {
                "bars_per_hour": 52000,
                "data_quality": 99.2
    ...
    def _serve_multi_panel_viz(self):
        """Serve multi-panel visualization data."""
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
