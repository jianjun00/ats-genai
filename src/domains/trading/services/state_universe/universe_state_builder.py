...
class UniverseStateIntervalBuilder(RunnerCallback):
    ...
    async def handleInterval(self, runner, current_time):
        ...
        for symbol, ohlc_data in ohlc_batch.items():
            if ohlc_data is None or (hasattr(ohlc_data, 'empty') and ohlc_data.empty):
                self.logger.warning(f"🔍 [DEBUG] Symbol {symbol}: Got None or empty data")
        ...
        for inst_id in instrument_ids:
            ...
            if ohlc is not None and not (hasattr(ohlc, 'empty') and ohlc.empty):
