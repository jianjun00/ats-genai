...
class UniverseMembershipDAO:
    ...
    async def initialize(self):
        ...
        try:
            ...
            print(f"[DAO ERROR] Failed to check schema: {e}")
            # Fall back to defaults if we can't check the schema
        ...
        try:
            async with pool.acquire() as conn:
                await conn.execute(f"""
                    ...
                    SET {self.end_date_col} = $3
                    WHERE universe_id = $1 AND instrument_id = $2 AND {self.end_date_col} IS NULL
    ...
    async def add_membership_full(self, universe_id: int, instrument_id: int, start_at=None, end_at=None):
        ...
        try:
            async with pool.acquire() as conn:
                await conn.execute(f"""
                    INSERT INTO {self.table_name} (universe_id, instrument_id, {self.start_date_col}, {self.end_date_col})
                    VALUES ($1, $2, $3, $4)
    ...
    async def add_membership(self, universe_id: int, symbol=None, instrument_id=None, start_at=None, end_at=None, vendor_id=None) -> bool:
        ...
        try:
            async with pool.acquire() as conn:
                ...
                await conn.execute(f"""
                    INSERT INTO {self.table_name} (universe_id, symbol, instrument_id, {self.start_date_col}, {self.end_date_col})
                    VALUES ($1, $2, $3, $4, $5)
    ...
    async def remove_membership(self, universe_id: int, symbol: str, start_at: datetime) -> bool:
        ...
        try:
            async with pool.acquire() as conn:
                result = await conn.execute(f"DELETE FROM {self.table_name} WHERE universe_id = $1 AND symbol = $2 AND {self.start_date_col} = $3", universe_id, symbol, start_at)
    ...
    async def get_active_memberships(self, universe_id: int, as_of):
        print(f"[DEBUG] get_active_memberships called with universe_id={universe_id}, as_of={as_of} (type={type(as_of)})")
        await self.initialize()
        pool = await asyncpg.create_pool(self.db_url)
        try:
            async with pool.acquire() as conn:
                result = await conn.fetch(
                    f"SELECT * FROM {self.table_name} WHERE universe_id = $1 AND {self.start_date_col} <= $2 AND ({self.end_date_col} IS NULL OR {self.end_date_col} > $2)",
                    universe_id, as_of)
                print(f"[DEBUG] get_active_memberships returned: {result}")
                return result
        finally:
            await pool.close()
