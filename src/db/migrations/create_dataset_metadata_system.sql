...
CREATE TABLE dev_dataset_columns (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE dev_dataset_column_stats (
    id SERIAL PRIMARY KEY,
...
COMMENT ON TABLE dev_datasets IS 'Unified metadata for all dataset types (tables, files, training data)';
COMMENT ON TABLE dev_dataset_columns IS 'Detailed column metadata and statistics for all datasets';  
COMMENT ON TABLE dev_dataset_column_stats IS 'Pre-computed histogram statistics for fast EDA visualization';

COMMENT ON COLUMN dev_datasets.dataset_type IS 'Type: database_table, single_file, sharded_files, training_dataset';
COMMENT ON COLUMN dev_datasets.stats_computed IS 'Whether comprehensive statistics have been computed';
