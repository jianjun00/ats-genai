-- Migration 051 INTG: Add Training Dataset Visualization Metadata for INTG Environment
-- Adds essential metadata fields for proper frontend visualization handling

-- Add visualization and data format metadata to INTG training datasets table
...
COMMENT ON COLUMN intg_training_dataset.data_format IS 'Data format type: csv_time_series, numpy_sequences, parquet, etc.';
COMMENT ON COLUMN intg_training_dataset.sequence_length IS 'Number of time steps per sequence (1 for time series, 60+ for sequences)';
COMMENT ON COLUMN intg_training_dataset.time_resolution IS 'Time resolution: minute, hourly, daily, weekly, monthly';
COMMENT ON COLUMN intg_training_dataset.visualization_type IS 'Visualization type: time_series, sequence_window, candlestick';
COMMENT ON COLUMN intg_training_dataset.time_step_unit IS 'What each step represents: hour, day, minute, time_step';
COMMENT ON COLUMN intg_training_dataset.is_time_series IS 'True if data is continuous time series, false if sequences';
COMMENT ON COLUMN intg_training_dataset.window_size IS 'Default window size for visualization (21 for 21-row window)';
COMMENT ON COLUMN intg_training_dataset.feature_metadata IS 'JSON metadata about features, OHLC columns, technical indicators';
...
CREATE INDEX IF NOT EXISTS idx_intg_training_dataset_time_resolution 
ON intg_training_dataset(time_resolution);

-- Update existing datasets with proper metadata based on detection heuristics
UPDATE intg_training_dataset SET 
    data_format = CASE 
...
-- Recreate view for easy visualization metadata access
