...
class UnifiedAnalyticsManager:
    ...
    # ===== ENHANCED DATASET DETAIL FUNCTIONALITY =====
    ...
    async def get_sequence_details(self, dataset_name: str, sequence_id: int) -> Dict[str, Any]:
        ...
        sequence_labels = labels[sequence_id] if len(labels.shape) > 2 else labels[sequence_id:sequence_id+1]
...
async def list_dataset_sequences(
    ...
    value_min: Optional[float] = Query(None, description="Minimum feature value"),
    value_max: Optional[float] = Query(None, description="Maximum feature value")
):
    """List sequences within a dataset with filtering capabilities."""
    ...
    try:
        return await analytics_manager.list_dataset_sequences(
            dataset_name, limit, offset, symbol_filter, feature_filter, value_min, value_max
        )
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error loading sequences: {e}")
...
async def get_sequence_details(dataset_id: int, sequence_id: int):
    ...
    try:
        return await analytics_manager.get_sequence_details(dataset_name, sequence_id)
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error loading sequence details: {e}")
...
async def get_sequence_ohlc(dataset_id: int, sequence_id: int):
    ...
    try:
        return await analytics_manager.get_sequence_ohlc_data(dataset_name, sequence_id)
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error loading sequence OHLC data: {e}")


@app.get("/api/v1/datasets/{dataset_id}/filtered-distributions")
async def get_filtered_distributions(
    dataset_id: int,
    sequence_ids: Optional[str] = Query(None, description="Comma-separated sequence IDs")
):
    ...
    try:
        return await analytics_manager.get_filtered_distributions(dataset_name, seq_ids)
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error calculating distributions: {e}")
...
async def health_check():
    return {"status": "healthy", "database": "connected", "features": ["jobs", "datasets", "sequences"]}
