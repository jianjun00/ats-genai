...
echo -e "${YELLOW}Updating Docker image with database connection changes${NC}"
...
docker run --rm dragonflyer762/ats-genai:dev-latest cat /app/src/config/database.py | grep "timescaledb.ats-dev.svc.cluster.local"
...
echo -e "${GREEN}Done! The Docker image has been updated with the database connection changes.${NC}"
