...
echo -e "${YELLOW}Updating Docker image with database retry mechanism${NC}"
...
docker run --rm dragonflyer762/ats-genai:dev-latest cat /app/src/config/database.py | grep "create_pool_with_retry"
docker run --rm dragonflyer762/ats-genai:dev-latest cat /app/src/config/db_retry.py | grep "retry_async"
...
echo -e "${GREEN}Done! The Docker image has been updated with the database retry mechanism.${NC}"
