...
async def test_comprehensive_multi_timeframe_training_data_pipeline(unit_test_db):
    ...
    try:
        ...
        try:
            ...
            print(f"   ⚠️ Technical signal verification failed: {tech_error}")
            print(f"      This may indicate missing indicator columns in database schema")
        ...
        # Handle expected real system issues
        ...
        if is_expected_error:
            print(f"\n✅ EXPECTED REAL SYSTEM ISSUE EXPOSED:")
            print(f"   🔍 Error: {e}")
            print(f"   📋 This demonstrates why real system testing is crucial")
            print(f"   🚫 Mock objects would have hidden this infrastructure dependency")
            
            # This is actually success - we found a real system issue
            pytest.fail(
                f"Real system infrastructure issue exposed: {e}. "
                "This test successfully identified a dependency that needs to be resolved."
            )
        else:
