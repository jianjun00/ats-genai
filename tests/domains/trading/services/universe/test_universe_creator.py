...
async def test_universe_add_remove(unit_test_db, monkeypatch):
    ...
    async with pool.acquire() as conn:
        columns = await conn.fetch(f"""
            ...
            WHERE table_name = '{daily_price_polygon_table}'
            ORDER BY ordinal_position
        """)
        print(f"[DEBUG] Schema for {daily_price_polygon_table}:")
        for col in columns:
            print(f"  {col['column_name']} {col['data_type']} nullable={col['is_nullable']}")
        indexes = await conn.fetch(f"""
            SELECT indexname, indexdef
            FROM pg_indexes
            WHERE tablename = '{daily_price_polygon_table}'
        """)
        print(f"[DEBUG] Indexes for {daily_price_polygon_table}:")
        for idx in indexes:
            print(f"  {idx['indexname']}: {idx['indexdef']}")
        # Print all rows before insert
        rows = await conn.fetch(f"SELECT date, symbol, instrument_id FROM {daily_price_polygon_table}")
        print(f"[DEBUG] Existing rows in {daily_price_polygon_table} before test insert: {len(rows)}")
        for row in rows:
            print(f"  {row}")
        # Clean up table before test insert
        await conn.execute(f"DELETE FROM {daily_price_polygon_table}")
        print(f"[DEBUG] Deleted all rows from {daily_price_polygon_table} before test insert.")
    # Insert daily prices
