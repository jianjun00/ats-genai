import pytest
# PL indicator does not exist, OneOneHigh, Z1B, Z2B, Z5T, Z6T, EnvelopeTop, EnvelopeBot
...
def test_oneonehigh_missing_ohlc():
    from domains.trading.services.indicators import OneOneHigh
    from domains.trading.services.state.instrument_interval import InstrumentInterval
    from datetime import datetime, timedelta
    base = datetime(2023, 1, 1)
    # Missing high
...
def test_oneonelow_ok(three_ok_intervals):
    from domains.trading.services.indicators import OneOneLow
    indicator = OneOneLow()
    indicator.update(three_ok_intervals)
    assert indicator.status == 'ok'
...
import pytest
# PL indicator does not exist, OneOneHigh, OneOneLow
from domains.trading.services.state.instrument_interval import InstrumentInterval
from datetime import datetime, timedelta
...
def test_etop_too_few():
    from domains.trading.services.indicators import ETop
    base = datetime(2023, 1, 1)
...
def test_ebot_too_few():
    from domains.trading.services.indicators import EBot
    base = datetime(2023, 1, 1)
