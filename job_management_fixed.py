#!/usr/bin/env python3
"""
Job Management API - Fixed to use existing dev_runs table

Minimal implementation that maps to existing dev_runs schema:
- job_type → run_type
- user_id → created_by  
- job_id → id
"""
...
# FastAPI app
app = FastAPI(title="Job Management (Fixed)", version="1.0.0")
...
async def list_jobs(
    ...
    return await job_manager.list_jobs(limit, offset, status)


@app.get("/health")
async def health():
    return {"status": "healthy", "database": "connected", "table": "dev_runs"}
...
<html>
<head>
    <title>Job Management (Fixed)</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 20px; }
        .job { border: 1px solid #ddd; padding: 10px; margin: 10px 0; border-radius: 5px; }
...
<body>
    <h1>🔧 Job Management (Fixed)</h1>
    <p>✅ Using existing dev_runs table</p>
    ...
    <script>
        async function load() {
            ...
            document.getElementById('stats').innerHTML = `
                <h3>Stats: ${stats.total_jobs} total, ${stats.running_jobs} running, ${stats.completed_jobs} completed, ${stats.failed_jobs} failed</h3>
            `;
            ...
            document.getElementById('jobs').innerHTML = 
                ...
                jobs.jobs.map(j => `
                    <div class="job">
                        ...
                        ${j.error_message ? '<br>Error: ' + j.error_message : ''}
                    </div>
                `).join('');
        }
