...
@pytest.mark.asyncio
async def test_shared_database_setup(backup_and_restore_tables, integration_test_db):
...
@pytest.mark.database
@pytest.mark.asyncio
async def test_test_fixtures_loading(backup_and_restore_tables, clean_integration_db):
    """Test loading test fixtures into integration database."""
    db_manager = TestDatabaseManager("integration")
    
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"clean_integration_db: {clean_integration_db}")
...
@pytest.mark.database
@pytest.mark.asyncio
async def test_data_isolation_between_tests(backup_and_restore_tables, clean_integration_db):
    ...
    try:
        async with pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                VALUES ($1, $2)
            """, 'test_isolation', 'Test isolation vendor')
...
@pytest.mark.database
@pytest.mark.asyncio
async def test_data_cleaned_from_previous_test(backup_and_restore_tables, clean_integration_db):
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_cross_table_relationships(backup_and_restore_tables, clean_integration_db):
    ...
    try:
        async with pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                VALUES ($1, 'TEST1', $2), ($1, 'TEST2', $2)
            """, universe_id, start_at)
