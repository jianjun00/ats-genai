#!/usr/bin/env python3
"""
Script to set up the database schema for the ATS system.
...
async def setup_database_schema():
    ...
    # Modify the URL to use dev_db instead of trading_db_dev
    db_url = db_url.replace('trading_db_dev', 'dev_db')
    
    # First, ensure the dev_db database exists
    try:
        # Connect to postgres database to create dev_db if it doesn't exist
        postgres_url = db_url.rsplit('/', 1)[0] + '/postgres'
        print(f"Connecting to postgres database: {postgres_url}")
        
        import asyncpg
        conn = await asyncpg.connect(postgres_url)
        
        # Check if dev_db exists
        exists = await conn.fetchval("SELECT 1 FROM pg_database WHERE datname = 'dev_db'")
        if not exists:
            print("Creating dev_db database...")
            await conn.execute("CREATE DATABASE dev_db")
            print("dev_db database created successfully")
        else:
            print("dev_db database already exists")
            
        await conn.close()
    except Exception as e:
        print(f"Error checking/creating database: {e}")
        print("Continuing with existing database...")
        # Continue with the script even if we can't create the database
