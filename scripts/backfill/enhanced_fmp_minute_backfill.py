...
- Polygon: 190M+ minute records ✅
- Tiingo: 160M+ minute records ✅  
- FMP: Only live data table - NO HISTORICAL MINUTE DATA ❌

This script creates comprehensive FMP minute data coverage for:
- 10,000 instruments
- 20 years historical (2005-2025)
- Enhanced error handling with exponential backoff
- Circuit breaker pattern for API protection
- Intelligent chunking for massive dataset
"""
...
class RetryConfig:
    ...
    max_retries: int = 3              # Conservative for minute data
    base_delay: float = 2.0           # 2 second base delay
    max_delay: float = 180.0          # 3 minute max delay
...
class CircuitBreakerConfig:
    ...
    failure_threshold: int = 15       # Higher threshold for minute data volume
    success_threshold: int = 5
    timeout_seconds: int = 600        # 10 minute timeout for minute data
...
class FMPMinuteBackfillConfig:
    ...
    # Chunking strategy for massive minute data
    batch_size: int = 5               # Small batches due to minute data volume
    max_concurrent_requests: int = 3   # Conservative concurrency
    save_progress_interval: int = 50   # Frequent saves for minute data
    checkpoint_file: str = "/tmp/fmp_minute_backfill_checkpoint.json"
    
    # Temporal chunking - critical for minute data
    days_per_chunk: int = 7           # Weekly chunks to manage API limits
    max_chunks_per_symbol: int = 1040  # 20 years * 52 weeks = ~1040 chunks
    
    # FMP minute API limits (more restrictive than daily)
    calls_per_minute: int = 100       # Conservative for minute data
    calls_per_day: int = 5000         # Lower limit for minute endpoints  
    base_delay: float = 0.6           # 100 calls/min = 0.6s between calls
    
    # Enhanced retry for minute data complexity
...
class MinuteBackfillProgress:
    ...
    total_chunks: int = 0              # Track chunks (symbol × time periods)
    completed_chunks: int = 0
    failed_symbols: List[str] = None
    skipped_symbols: List[str] = None
    failed_chunks: List[str] = None    # Track failed time chunks
...
class EnhancedFMPMinuteBackfiller:
    ...
    async def exponential_backoff_delay(self, attempt: int, status_code: int) -> float:
        ...
        if retry_config.jitter:
            jitter_range = delay * 0.15  # 15% jitter for minute data
            delay += random.uniform(-jitter_range, jitter_range)
        
        # Enhanced handling for minute data API limits
        if status_code == 429:  # Rate limited - critical for minute data
            delay = max(delay, 120.0)  # Minimum 2 minutes
        elif status_code == 403:  # Quota exceeded - common with minute data
            delay = max(delay, 60.0)   # Minimum 1 minute
        elif status_code >= 500:
            delay = max(delay, 10.0)   # Minimum 10 seconds for server errors
    ...
    async def get_target_symbols(self) -> List[str]:
        """Get all 10k instruments for FMP minute backfill"""
        ...
        try:
            async with pool.acquire() as conn:
                ...
                self.logger.info(f"Found {len(symbols)} instruments for FMP minute backfill")
    ...
    async def run_fmp_minute_backfill(self) -> MinuteBackfillProgress:
        ...
        # Calculate total chunks for 20-year minute data
        ...
        self.logger.info(f"🚀 Starting Enhanced FMP Minute Backfill")
        self.logger.info(f"📅 Period: {self.config.start_date} to {self.config.end_date}")
        self.logger.info(f"🎯 Symbols: {len(all_symbols):,}")
        ...
        self.logger.info(f"⏱️  Estimated time: {self.progress.total_chunks * 0.6 / 3600:.1f} hours")
        
        # Create session for minute data
        timeout = aiohttp.ClientTimeout(total=120)  # Longer timeout for minute data
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced FMP Minute Backfill (20 Years)')
    parser.add_argument('--start-date', type=str, default='2005-01-01', help='Start date (YYYY-MM-DD)')
    parser.add_argument('--end-date', type=str, default='2025-08-23', help='End date (YYYY-MM-DD)')
    parser.add_argument('--batch-size', type=int, default=5, help='Symbols per batch')
    parser.add_argument('--days-per-chunk', type=int, default=7, help='Days per API call chunk')
