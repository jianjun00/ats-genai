#!/usr/bin/env python3
"""
EODHD Daily Price Backfill Script for ats-dev environment.
Fetches 30 years of daily price data for instruments using EODHD API.

Based on existing daily_price_backfill.py pattern with idempotent UPSERT operations.
"""
...
logger = logging.getLogger("eodhd_daily_price_backfill")

BASE_URL = "https://eodhistoricaldata.com/api/eod/{symbol}.US?api_token={api_key}&from={start}&to={end}&period=d&fmt=json"

def download_prices_eodhd(ticker, start, end, api_key):
    """Download daily prices from EODHD API."""
    url = BASE_URL.format(symbol=ticker, start=start, end=end, api_key=api_key)
...
async def insert_daily_prices_eodhd(pool, env, ticker, instrument_id, prices):
    """Insert daily prices into the EODHD database table."""
    if not prices:
        return 0
    
    table_name = env.get_table_name('daily_prices_eodhd')
    
    async with pool.acquire() as conn:
        rows = []
        for price in prices:
            try:
                date_val = dt.datetime.strptime(price['date'], '%Y-%m-%d').date()
                rows.append((
                    date_val,
                    ticker,
                    price['open'],
                    ...
                    price['close'],
                    price.get('volume', 0),
                    price.get('adjusted_close', price['close']),  # Use adjusted_close if available
                    instrument_id
                ))
            except Exception as e:
        ...
        # Insert with ON CONFLICT handling for idempotency
        try:
...
async def ensure_eodhd_table_exists(pool, env):
    """Ensure the EODHD daily prices table exists with proper schema."""
    table_name = env.get_table_name('daily_prices_eodhd')
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_{table_name}_instrument_id 
            ON {table_name} (instrument_id, date DESC)
        """)
        
        logger.info(f"EODHD table {table_name} ready")

async def get_instruments_to_backfill_eodhd(pool, env, limit=None):
    """Get list of instruments that need EODHD price backfill."""
    async with pool.acquire() as conn:
...
async def check_existing_prices_eodhd(pool, env, instrument_id, start_date, end_date):
...
async def backfill_daily_prices_eodhd(pool, env, eodhd_api_key, start_date, end_date, limit=None, skip_existing=True):
    """
    Backfill daily prices for instruments using EODHD API.
    ...
    await ensure_eodhd_table_exists(pool, env)
    
    # Get instruments to process
    instruments = await get_instruments_to_backfill_eodhd(pool, env, limit)
    ...
    for instrument in instruments:
        ...
        try:
            ...
            if skip_existing:
                existing_count = await check_existing_prices_eodhd(pool, env, instrument_id, start_date, end_date)
                if existing_count > 0:
            ...
            # Download prices from EODHD
            prices = download_prices_eodhd(
                symbol, 
            ...
            inserted_count = await insert_daily_prices_eodhd(pool, env, symbol, instrument_id, prices)
            total_prices_inserted += inserted_count
            total_processed += 1
            
            logger.info(f"Completed {symbol}: {inserted_count} prices inserted")
            
            # Rate limiting - EODHD free tier allows 20 calls/minute
            time.sleep(3.0)  # 20 calls per minute max
            
        except Exception as e:
            logger.error(f"Failed to process {symbol}: {e}")
            continue
    
    logger.info(f"EODHD backfill complete. Processed {total_processed} instruments, inserted {total_prices_inserted} total prices")

async def main():
    parser = argparse.ArgumentParser(description="Backfill daily prices from EODHD API")
    ...
    parser.add_argument('--limit', type=int, default=int(os.getenv('LIMIT', '0')) or None, help='Limit number of instruments to process')
    parser.add_argument('--years', type=int, default=int(os.getenv('YEARS', '30')), help='Number of years of historical data to fetch (default: 30)')
    ...
    try:
        ...
        if not eodhd_api_key:
            logger.error("No EODHD API key found. Set EODHD_API_KEY environment variable or configure in Gin file.")
            sys.exit(1)
        ...
        # Run backfill
        await backfill_daily_prices_eodhd(
            pool, env, eodhd_api_key, start_date, end_date, 
            limit=args.limit, skip_existing=args.skip_existing
        )
        
        await pool.close()
        logger.info("EODHD daily price backfill complete")
        
    except Exception as e:
        logger.error(f"Failed to run EODHD daily price backfill: {e}")
        import traceback
