...
run_migration.py: |
"""
Run enhanced runs table migration
"""
import asyncio
import asyncpg
import logging
...
# Read migration SQL
with open('/scripts/migration.sql', 'r') as f:
migration_sql = f.read()

# Execute migration
await conn.execute(migration_sql)
logger.info("✅ Enhanced runs table migration completed successfully")

# Verify new columns were added
table_info = await conn.fetch("""
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'dev_runs' 
AND column_name LIKE '%summary%'
ORDER BY column_name;
""")

logger.info(f"📋 Added summary columns:")
for col in table_info:
logger.info(f"  ✓ {col['column_name']} ({col['data_type']})")

# Test the new view
view_test = await conn.fetchval("SELECT COUNT(*) FROM dev_price_unification_summary")
logger.info(f"📊 Price unification summary view has {view_test} records")

print("🎉 ENHANCED RUNS TABLE MIGRATION COMPLETED SUCCESSFULLY!")
