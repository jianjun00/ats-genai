...
class WeeklyMaintenance:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"❌ Failed to initialize weekly maintenance: {e}")
            raise
        ...
        async with self.db_pool.acquire() as conn:
            ...
            for vendor in vendors:
                ...
                try:
                    ...
                    logger.error(f"Error analyzing {vendor} data quality: {e}")
    ...
    async def cleanup_orphaned_records(self) -> MaintenanceResult:
        ...
        try:
            ...
            logger.error(f"❌ Orphaned record cleanup failed: {e}")
            ...
            async with self.db_pool.acquire() as conn:
                ...
                for table in tables_to_analyze:
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to vacuum {table}: {e}")

                # Reindex if needed (for tables with heavy inserts/updates)
                ...
                for table in price_tables:
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to check indexes for {table}: {e}")
            ...
            for log_dir in log_dirs:
                if Path(log_dir).exists():
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to cleanup {log_dir}: {e}")
                        result.errors.append(f"Storage cleanup error in {log_dir}: {e}")
            ...
            async with self.db_pool.acquire() as conn:
                ...
                for vendor in vendors:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to get stats for {vendor}: {e}")
                        vendor_stats[vendor] = {'error': str(e)}
    ...
    async def send_maintenance_summary(self, report: Dict):
        ...
        try:
            ...
            logger.error(f"❌ Error sending maintenance summary: {e}")
